...
const EnergyChart: React.FC<Props> = ({ location, from, to }) => {
  const { data, loading, error } = useQuery<EnergyStatsData, QueryVars>(
    AGGREGATE_ENERGY,
    {
      variables: {
        location,
  ...
  if (loading) return (
    <div className="h-[350px] flex items-center justify-center bg-slate-50 rounded-xl border border-dashed border-slate-300 animate-pulse">
      <span className="text-slate-400 font-medium">Loading energy consumtion data...</span>
    </div>
  );
  ...
  return (
    <div className="p-6 bg-white rounded-2xl shadow-sm border border-slate-200 mb-10">
      <div className="flex justify-between items-start mb-6 mt-6">
        <div>
          <p className="text-sm text-slate-500" style={{color: 'white', fontStyle: 'bold'}}>Location: {location}</p>
        </div>
      </div>
